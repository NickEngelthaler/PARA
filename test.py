...
import sys 
import os
dir = "/Users/yixiang-xu/Research/TPL/code/8v_TPL_functions_Intensity_Generator"

sys.path.append(dir)
os.chdir('/Users/yixiang-xu/Research/TPL/code/8v_TPL_functions_Intensity_Generator')
...
import pandas as pd 
data = pd.read_csv('/Users/yixiang-xu/Research/TPL/code/Pratik-Optimize-code/Optimize-main/parallel/uncoded_consumer_tweets_dataframe_small.csv')
import time
start = time.time()
for i in range(data.shape[0]):
	temp = PARA.run(data['tweet'].iloc[i])
	print(i)
...
with concurrent.futures.ProcessPoolExecutor() as executor:
    for i in executor.map(PARA.run,data['tweet']):
        results.append(i)
        count += 1     
...
import sys 
import os
dir = "/Users/yixiang-xu/Research/TPL/code/7v_TPL_functions_Intensity_Generator"

sys.path.append(dir)
os.chdir('/Users/yixiang-xu/Research/TPL/code/7v_TPL_functions_Intensity_Generator')
...
start = time.time()
for i in range(data.shape[0]):
	temp = TPL_Int_Score(dir, data['tweet'].iloc[i], brown_dic)
	print(i)
...
with concurrent.futures.ProcessPoolExecutor() as executor:
    for i in executor.map(do,data['tweet']):
        results.append(i)
...
start = time.time()
for i in range(data.shape[0]):
	temp = Detect_Emojicon_TPL(data['tweet'].iloc[i], key_emoji, key_emoticon)
